...
class Scanner {
  ...
  static {
    ...
    keywords.put("simulate", SIMULATE);
    keywords.put("policy", POLICY);
    keywords.put("threshold", THRESHOLD);
    keywords.put("default", DEFAULT);
    keywords.put("rainfall", RAINFALL);
    keywords.put("maxflow", MAXFLOW);
  ...
  private void number() {
    ...
    // Check if this is a multiplier token (L suffix)
    if (peek() == 'L') {
      advance();
      addToken(MULTIPLIER,
    ...
    // Check if this is a rainfall token (R followed by number) - KEEP FOR BACKWARD COMPAT
    if (peek() == 'R') {
      advance();
      addToken(RAINFALL,
          Double.parseDouble(source.substring(start, current-1)));
  ...
  private void quotedString() {
    ...
    if (isAtEnd()) {
      Lox.error(line, "Unterminated quoted string.");
      return;
    }
